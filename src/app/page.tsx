...
import { FundoPraiaFixo } from "@/components/FundoPraia";
...
function Aviso({
  ...
  const estilos = {
    atencao: "border-coral-400/50 bg-coral-500/25",
    info: "border-mar-200/40 bg-mar-500/20",
    bom: "border-folha-500/50 bg-folha-500/25",
  }[tom];

  return (
    <li className={`rounded-2xl border ${estilos} px-4 py-3`}>
      <p className="text-sm font-semibold text-white">{titulo}</p>
      <p className="mt-0.5 text-sm text-white/85">{children}</p>
    </li>
  );
}
...
function Numero({
  ...
  return (
    <div className="rounded-2xl border border-white/25 bg-oceano-900/35 px-4 py-3 backdrop-blur-md">
      <p className="text-xs font-semibold tracking-wide text-white/65 uppercase">{titulo}</p>
      <p className="font-display text-xl text-white">{valor}</p>
      {detalhe && <p className="mt-0.5 text-xs text-white/70">{detalhe}</p>}
    </div>
  );
}
...
function Legenda({ cor, children }: { cor: string; children: React.ReactNode }) {
  return (
    <span className="flex items-center gap-1.5 rounded-full bg-oceano-900/40 px-2.5 py-1 ring-1 ring-white/25 backdrop-blur-sm">
...
function Coluna({ rotulo, valor }: { rotulo: string; valor: string }) {
  return (
    <div className="rounded-xl border border-white/20 bg-white/10 px-3 py-2">
      <p className="text-[11px] text-white/70">{rotulo}</p>
    ...
    <>
      ...
      <div className="space-y-6">
        <header className="text-center">
          ...
          <h1 className="mt-1.5 font-display text-3xl leading-tight text-white drop-shadow-[0_3px_14px_rgba(8,37,46,0.6)] sm:text-5xl">
            {viagem.nome}
          </h1>
          {viagem.descricao && (
          ...
          <div className="mt-3 flex flex-wrap justify-center gap-2">
            {datasAtivas.map((data) => (
              <span
                key={data.id}
                className="rounded-full bg-oceano-900/40 px-3 py-1.5 text-xs font-semibold text-white ring-1 ring-white/25 backdrop-blur-sm"
              >
                {dataCurta(data.inicio)} → {dataCurta(data.fim)} · {pluralizar(data.noites, "noite", "noites")}
              </span>
            ))}
            <span className="rounded-full bg-oceano-900/40 px-3 py-1.5 text-xs font-semibold text-white ring-1 ring-white/25 backdrop-blur-sm">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 text-[11px] font-semibold text-white">
          ...
          {vagasLivres > 0 && (
            <Legenda cor="bg-white/25">{pluralizar(vagasLivres, "vaga livre", "vagas livres")}</Legenda>
          )}
        </div>

        <Link
          href="/casas"
          className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 rounded-2xl border border-white/25 bg-oceano-900/40 px-4 py-3 text-center text-sm text-white backdrop-blur-md transition hover:bg-oceano-900/55"
        >
          <span className="font-semibold">{casa ? casa.nome : "Ainda não temos casa escolhida"}</span>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            <Link href="/rateio" className="botao-vidro w-full">
              Ver rateio
            </Link>
          </div>
        </div>
          ...
          <section className="vidro overflow-hidden p-0">
            <div className="flex flex-col sm:flex-row">
              <div className="relative h-40 w-full shrink-0 sm:h-auto sm:w-56">
                ...
                <span className="selo absolute top-3 left-3 bg-oceano-900/80 text-white ring-1 ring-white/30">
                  {ROTULO_CASA[origemCasa]}
                </span>
              </div>

              <div className="min-w-0 flex-1 p-5">
                <h2 className="font-display text-xl leading-snug text-white">{casa.nome}</h2>
                <p className="mt-0.5 text-sm text-white/75">{casa.regiao || "Búzios, RJ"}</p>

                <div className="mt-3 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
                  <Coluna rotulo="Hóspedes" valor={`${casa.capacidade || "?"}`} />
                  <Coluna rotulo="Quartos" valor={`${casa.quartos || "?"}`} />
                  <Coluna
                    rotulo="Total"
                    valor={casa.precoAConfirmar ? "a confirmar" : reaisCurto(casa.valorTotal)}
                  />
                  <Coluna
                    rotulo="Por adulto"
                    valor={valorPorAdulto > 0 ? reaisCurto(valorPorAdulto) : "a calcular"}
                  />
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {casa.link && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="botao-vidro px-4 py-2 text-sm"
                    >
                      Abrir no Airbnb
                    </a>
                  )}
        ...
        <section className="vidro">
          <h2 className="font-display text-xl text-white">Avisos importantes</h2>
          <ul className="mt-3 space-y-2">
            ...
            {valorPorAdulto > 0 && (
              <Aviso tom="info" titulo="Valor estimado hoje">
                {reais(valorPorAdulto)} por adulto, considerando{" "}
                {pluralizar(resumo.confirmados, "pessoa confirmada", "pessoas confirmadas")}
            ...
            {resumo.pendentes > 0 && (
              <Aviso tom="info" titulo="Ainda faltam respostas">
                {pluralizar(
                  resumo.pendentes,
                  "pessoa da lista ainda não respondeu",
                  "pessoas da lista ainda não responderam",
                )}
                .{" "}
                <Link href="/presenca" className="font-semibold text-white underline decoration-dotted">
                  Ver quem falta
                </Link>
              </Aviso>
            )}
        ...
        {preferencias.length > 0 && (
          <section className="vidro">
            <h2 className="font-display text-xl text-white">Preferência de datas</h2>
              ...
              {preferencias.map((data) => (
                <li key={data.id}>
                  <div className="flex items-baseline justify-between gap-3 text-sm">
                    <span className="font-medium text-white">{data.rotulo}</span>
                    <span className="text-white/75">{pluralizar(data.votos, "pessoa", "pessoas")}</span>
                  </div>
                  <div className="mt-1.5 h-2.5 overflow-hidden rounded-full bg-white/20">
                    <div
                      className="h-full rounded-full bg-mar-300"
                      style={{
                        width: `${
        ...
        {ranking.some((item) => item.votos > 0) && (
          <section className="vidro">
            <div className="flex items-baseline justify-between gap-3">
              <h2 className="font-display text-xl text-white">Como está a votação</h2>
              <Link href="/casas" className="text-sm font-semibold text-white underline decoration-dotted">
                votar
              </Link>
            </div>
            <ol className="mt-3 space-y-2">
              {ranking
                ...
                .map((item, indice) => (
                  <li key={item.id} className="flex items-center gap-3 text-sm text-white">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-white/20 font-display text-sm">
                      {indice + 1}
                    </span>
        ...
        {confirmados.length > 0 && (
          <section className="vidro">
            <div className="flex items-baseline justify-between gap-3">
              <h2 className="font-display text-xl text-white">Já estão dentro</h2>
              <Link href="/presenca" className="text-sm font-semibold text-white underline decoration-dotted">
                ver todos
              </Link>
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              {confirmados.map((amigo) => (
                <Link
                  ...
                  className="flex items-center gap-2 rounded-full border border-white/25 bg-white/10 py-1 pr-3 pl-1 text-sm text-white transition hover:bg-white/20"
                >
                  <Avatar
                    nome={amigo.nome}
                    sobrenome={amigo.sobrenome}
